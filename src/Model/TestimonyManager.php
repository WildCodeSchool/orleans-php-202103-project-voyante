...
class TestimonyManager extends AbstractManager
{
    ...
    {   
        $query = "INSERT INTO " . self::TABLE . "('name', 'mail', 'message', 'validation') VALUES (:name, :mail, :message, :validation)";
    ...
    {
        $query = "UPDATE " . self::TABLE . " SET 'name'=:name, 'mail'=:mail, 'message'=:message, 'validation'=:validation WHERE id=:id";
        $statement = $this->pdo->prepare($query);
        $statement->bindValue('name', $item['name'], \PDO::PARAM_STR);
        $statement->bindValue('mail', $item['mail'], \PDO::PARAM_STR);
        $statement->bindValue('message', $item['message'], \PDO::PARAM_STR);
